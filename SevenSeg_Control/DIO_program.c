...
{
	u8 Local_u8Realing[10];

	for (u8 i=0;i<9;i++){

		switch(Copy_u8Port)
		{
		case PORTA_REG : Local_u8Realing[i]=GET_BIT(PINA,i);break;
		case PORTB_REG : Local_u8Realing[i]=GET_BIT(PINB,i);break;
		case PORTC_REG : Local_u8Realing[i]=GET_BIT(PINC,i);break;
		case PORTD_REG : Local_u8Realing[i]=GET_BIT(PIND,i);break;
		}
	}
	return Local_u8Realing;
}
